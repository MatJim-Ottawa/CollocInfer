...
% CollocInfer. 
%  We use the SEIR equations, various modifications of which are used to 
%  model the spread of disease in epidemeiology.  The letters stand for:
%
%  Susceptibles:  the large subpopulation of those who have not been 
%                 infected
%  Exposed:       the smaller subpopulation of those who have been 
%                 infected, but are not yet infectious
%  Infectious:    those who have already been infected, and are now also 
%                 able to infect others
%  Recovered:     these were infected, but are now recovered and are, 
%                 usually, immune to further infections.
%
%  The equations are used to model measles infections in the Canadian 
%  province of Ontario over a two-year period.  The data analyzed here are 
%  derived froma larger dataset analyzed in greater detail by Hooker, 
%  Ellner, De Vargas Roditi and Earn (2011).  
%
%  The basic equations are here modified to all for a seasonally varying
%  infection rate, a realistic feature for modeling childhood diseases 
%  where the infection rate increases during the school year.  The function 
%  that provides the seasonal variability is beta(t).
%
...
% computational efficiency and positivity argue for fitting the logarithm  
% of the data, and also modelling the logarithm of each variable. 
% This requires a simple modification of the SEIR equations above, 
% consisting of dividing the right side of the equations by the exponential  
% of the respective variables.
% The CollocInfer package contains a number of useful tools that let you  
...
%  factor for converting natural logs to common logs
...
%  The first five should be positive, so it would be smarter to\
%  estimate their logs.  Change the equations to reflect this.

% SEIRlogpars = SEIRpars; 
% SEIRlogpars(1:5) = log(SEIRlogpars(1:5));
...
SEIRdata = [nan.*ones(length(Idata),2),Idata];
...
SEIRlogdata = log(SEIRdata);

% Function make_SEIR is distributed with the package, and defines 
...
% viewed by typing "make_SEIR" into the command window in Matlab.
...
SEIRbasis = create_bspline_basis(rr,nbasis,norder,knots);
...
DEfd = smooth_basis(SEIRtimes,SEIRlogdata(:,3),fdPar(SEIRbasis,1,0.1));

% plot the smooth log data

plotfit_fd(SEIRlogdata(:,3), SEIRtimes, DEfd)
...
%  coefficients on raw data scale

% SEIRcoefs0      = [ones(nbasis,2), getcoef(DEfd)];
% SEIRcoefs0(:,1) = SEIRcoefs0(:,1).*exp(13);
% SEIRcoefs0(:,2) = SEIRcoefs0(:,2).*exp(7);

%  coefficients on log data scale

SEIRcoefs0      = [ones(nbasis,2), getcoef(DEfd)];
SEIRcoefs0(:,1) = SEIRcoefs0(:,1).*13;
SEIRcoefs0(:,2) = SEIRcoefs0(:,2).*7;

% set up the functional data object for the three variables

DEfd0 = fd(SEIRcoefs0,SEIRbasis);

% We use the usual least squares measures of the fidelity of the estimated
% functions S(t), E(t) and I(t) to the data and to the differential 
% equation.  This means that we can save the user the effort of setting up 
% the lik and proc objects by calling function LS.setup, which output these 
% objects.

% We will want to represent the state variables on the log scale so that  
% they remain always positive. To do this, we set the 'posproc' argument of 
...
tic;
SEIRcoefs1 = FitMatchOpt(SEIRcoefs0, which, SEIRpars, SEIRproc, ...
                         options_in);
toc

%  0.8 seconds, 124 iterations
...
DEfitfd1 = fd(SEIRcoefs1(:,3),SEIRbasis);

figure(1)
plotfit_fd(SEIRlogdata(:,3),SEIRtimes,DEfitfd1)
xlabel('\fontsize{13} Year')
ylabel('\fontsize{13} Log number infected')
...
% setting turned off rather then when turned on.  

%  define the new function estimates

DEfitfd2 = fd(SEIRcoefs2(:,3),SEIRbasis);
...
%  This version of active only optimizes i coefficients in additiion to
%  beta coefficients.
...
tic;
[SEIRpars3, SEIRcoefs3, value, gradient] = ...
    outeropt(SEIRtimes, SEIRlogdata, SEIRcoefs2, SEIRpars, ...
             SEIRlik, SEIRproc, ...
             active, [], control_in, [], control_out);
toc

%  59.4 secs, 5 iterations, terminated because could not decrease
...
INNEROPT_COEFS0 = SEIRcoefs3; 

tic;
Profile_LS_struct = ...
    Profile_LS(SEIRfn, SEIRtimes, SEIRlogdata, SEIRcoefs3, ...
               SEIRpars, SEIRbasis, ...
               SEIRlambda, DEfd0, betamore, [], [], active, ...
               [], control_in, [], control_out, [], poslik, posproc);
toc

%   60 secs, 100 iterations, exceeded max iterations
%  214 secs, 400 iterations, exceed max fn. evals

SEIRpars4  = Profile_LS_struct.pars;
SEIRcoefs4 = Profile_LS_struct.coefs;

% Some plots

%  display the initial and estimated parameter values

[SEIRpars4(active); SEIRpars3(active)]

% Plot the estimated trajectories

DEfitfd4 = fd(SEIRcoefs4,SEIRbasis);
plot(DEfitfd4)
xlabel('\fontsize{13} Year')
ylabel('\fontsize{13} Log number infected')
...
figure(1)
SEIRlogSfit = eval_fd(SEIRtimes, DEfitfd4(1));
phdl = plot(SEIRtimes,exp(SEIRlogdata(:,1)), 'bo', ...
            SEIRtimes,exp(SEIRlogSfit), 'b-');
set(phdl, 'LineWidth', 2)
xlabel('\fontsize{16} Year')
ylabel('\fontsize{16} Number susceptible')

figure(2)
SEIRlogEfit = eval_fd(SEIRtimes, DEfitfd4(2));
phdl = plot(SEIRtimes,exp(SEIRlogdata(:,2)), 'bo', ...
            SEIRtimes,exp(SEIRlogEfit), 'b-');
set(phdl, 'LineWidth', 2)
xlabel('\fontsize{16} Year')
ylabel('\fontsize{16} Number exposed')

figure(3)
SEIRlogIfit = eval_fd(SEIRtimes, DEfitfd4(3));
phdl = plot(SEIRtimes,exp(SEIRlogdata(:,3)), 'bo', ...
            SEIRtimes,exp(SEIRlogIfit), 'b-');
set(phdl, 'LineWidth', 2)
xlabel('\fontsize{16} Year')
ylabel('\fontsize{16} Number infected')

% We can also look at the discrepancy between the estimated trajectory and 
% the differential equation

traj = eval_fd(SEIRtimes,DEfitfd4);     % estimated trajectory

% derivative of the estimated trajectory

dtraj = eval_fd(SEIRtimes,DEfitfd4,1);  

% Trajectory predicted by ODE

ftraj = SEIRproc.more.fn(SEIRtimes,traj,SEIRpars3,SEIRproc.more.more);   

plot(SEIRtimes, dtraj, 'b-', SEIRtimes, ftraj, 'r--')
xlabel('\fontsize{13} Year')
ylabel('SEIR derivatives' )
title('\fontsize{16} blue ... deriv. x(t);      red ... RS of DE')

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% The alternative is to exponentiate the state before we compare it to 
%  the original data.  This can take a very long time and is only 
%  recommended if you really need to do it, or have a couple of hours
%  to wait.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
SEIRcoefs4 = inneropt(SEIRtimes, data, SEIRpars3, ...
                      SEIRlik2, SEIRproc2, [], control_in);
...
[SEIRpars5, SEIRcoefs5, value, gradient] = ...
    outeropt(SEIRtimes, SEIRlogdata, SEIRcoefs4, SEIRpars3, ...
             SEIRlik2, SEIRproc2, ...
             active, [], control_in, [], control_out);
...
%
% Here we go through the steps necessary to set up the SEIR equations  
% manually.  This allows us several options for dealing with the positivity  
% of the state vector. 
%
% 1. We can ignore it and hope for the best. 
%
% 2. We can take a log transform of the ODE and then exponentiate the 
% solutions to compare to the data
%
% 3. We can take a log transform of the ODE and compare this to the log 
%    data. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


% First of all, we need the values of the basis at the observation times  
% and the quadrature points. 

qpts = 0.5*(knots(1:(length(knots)-1)) + knots(2:length(knots)));

bvals.obs = eval_basis(SEIRtimes,SEIRbasis);

bvals.bvals  = eval_basis(qpts,SEIRbasis);
bvals.dbvals = eval_basis(qpts,SEIRbasis,1);
...
%%% However, ODEs are often much more numerically stable if represented on 
% a log scale. The make_logtrans function will take the right hand side 
% functions and derivatives defined for any differential equation and 
% provide the equivalent system for the log state. Note that this does 
% affect the way  you represent your state when considering the 
% observations. 
...
slik               = make_SSElik;
slik.bvals         = eval_basis(SEIRtimes,SEIRbasis);
slik.more          = make_id;
slik.more.weights  = ones(size(data));
slik.more.names    = SEIRvarnames;
slik.more.parnames = SEIRparnames;

% Log transform transformation for the lik object. For this, we note that  
% we have represented the trajectory on the log scale and will need to  
% transform back, this makes the numerics much harder and it can take a 
...
dfd = smooth_basis(SEIRtimes,SEIRlogdata(:,3),SEIRbasis);

SEIRcoefs0 = zeros(nbasis,3);
SEIRcoefs0(:,3) = getcoef(dfd);
 
SEIRcoefs1 = FitMatchOpt(SEIRcoefs, which, SEIRpars3, lsproc, control_in);
...
SEIRcoefs2 = inneropt(SEIRtimes, SEIRlogdata, SEIRpars, ...
                      lslik, lsproc, [], control_in);

res3 = outeropt(SEIRtimes,data,res2.SEIRcoefs,SEIRpars,lslik,lsproc);
...
SEIRcoefs2 = inneropt(SEIRtimes, SEIRlogdata, SEIRpars3, ...      
                      lslike, lsproc, [], control_in);
...
[SEIRpars3, SEIRcoefs3, value, gradient] = ...
    outeropt(SEIRtimes, SEIRlogdata, SEIRcoefs2, SEIRpars, ...
             lslik, lsproc, active, [], control_in, [], control_out);
